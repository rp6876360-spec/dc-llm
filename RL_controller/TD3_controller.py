...
class ActorAdj(BasePolicy):
    ...
    def forward(self, obs: th.Tensor) -> th.Tensor:
        ...
        # For dc_is_change_range is True
        mkt_decision = features[:, -self.action_dim:] # range [0, 1], sum=1, may try to constrain the range of each dim is [0, 1], but the sum is not 1 (extract the feature before softmax in the market observer)
        final_output = (td3_decision + mkt_decision) - 1 # range [-1, 1]

        # For dc_is_change_range is False
        # mkt_decision = features[:, -self.action_dim:] # range [-1, 1],
        # final_output = (((td3_decision + mkt_decision) + 1) * 2 / 3) - 1 # range [-1, 1]
...
class ActorAdj_unChange(ActorAdj):
    def forward(self, obs: th.Tensor) -> th.Tensor:
        # assert deterministic, 'The TD3 actor only outputs deterministic actions'
        features = self.extract_features(obs)
        td3_decision = self.mu(features[:, :-self.action_dim]) # range [0, 1], sum=1
...
class TD3PolicyAdjUnchange(TD3Policy):
    def make_actor(self, features_extractor: Optional[BaseFeaturesExtractor] = None) -> ActorAdj:
        actor_kwargs = self._update_features_extractor(self.actor_kwargs, features_extractor)
        return ActorAdj_unChange(**actor_kwargs).to(self.device)
...
class TD3Controller(OffPolicyAlgorithm):
    ...
    policy_aliases: Dict[str, Type[BasePolicy]] = {
        ...
        'TD3PolicyAdjUnchange': TD3PolicyAdjUnchange,
