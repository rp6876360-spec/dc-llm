...
class MarketObserver_Algorithmic:
    def __init__(self, config, action_dim, **kwargs):
        self.config = config
        self.action_dim = action_dim # The dim of the hidden vector.
        input_kwargs = {'action_dim': self.action_dim, **kwargs}
...
@register_mkt_obs_model
def SentimentMulti_1(config, **kwargs):
    model = SENTIMENTMULTI_1(config, **kwargs)
    return model
class SENTIMENTMULTI_1:
    def __init__(self, config, **kwargs):
        super(SENTIMENTMULTI_1, self).__init__()
        self.name = 'SentimentMulti_1'
        self.config = config
        self.output_action_dim = kwargs['action_dim'] # for hidden vector to RL agent
        self.senti_score = kwargs['sentiment_score']

    def __call__(self, **kwargs):
        # (+1)/2 -> norm 
        # date and stock order
        curday = kwargs['cur_date']
        ...
        curscore = curdata['score'].values
 
        # sort stock, the stock order should be the same as the order in the env.
        if self.config.dc_is_change_range:
            curscore = (curscore + 1.0) * 0.5 # [-1, 1] -> [0, 1]
        if self.config.dc_is_norm_score:
            if np.sum(np.abs(curscore)) == 0:
                cur_hidden_vector_ay = np.array( [1/len(curscore)] * len(curscore) )
            else:
                cur_hidden_vector_ay = curscore / np.sum(np.abs(curscore))
        else:
            cur_hidden_vector_ay = curscore
        cur_hidden_vector_ay = cur_hidden_vector_ay * self.config.senti_k

        cur_hidden_vector_ay = np.reshape(cur_hidden_vector_ay, (1, -1))
        up_cnt = np.sum(curscore > 0.5)
        hold_cnt = np.sum(curscore == 0.5)
        down_cnt = np.sum(curscore < 0.5)
        # 0-up, 1-hold, 2-down
        sigma_val_ay = [np.argmax([up_cnt, hold_cnt, down_cnt])]
        lambda_val_ay = sigma_val_ay
        # hidden_vec: (batch, num_of_stocks) is the hidden vector sent to RL agents.
        # sigma_val_ay: (batch, )
        # lambda_val_ay: (batch, )
...
@register_mkt_obs_model
def SentimentBenzinga_1(config, **kwargs):
    model = SENTIMENTBENZINGA_1(config, **kwargs)
    return model
class SENTIMENTBENZINGA_1:
    def __init__(self, config, **kwargs):
        super(SENTIMENTBENZINGA_1, self).__init__()
        self.name = 'SentimentBenzinga_1'
        self.config = config
        self.output_action_dim = kwargs['action_dim'] # for hidden vector to RL agent
        self.senti_score = kwargs['sentiment_score']
...
@register_mkt_obs_model
def SentimentNonBenzinga_1(config, **kwargs):
    model = SENTIMENTNONBENZINGA_1(config, **kwargs)
    return model
class SENTIMENTNONBENZINGA_1:
    def __init__(self, config, **kwargs):
        super(SENTIMENTNONBENZINGA_1, self).__init__()
        self.name = 'SentimentNonBenzinga_1'
        self.config = config
        self.output_action_dim = kwargs['action_dim'] # for hidden vector to RL agent
        self.senti_score = kwargs['sentiment_score']
