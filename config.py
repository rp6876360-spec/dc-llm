...
class Config():
    def __init__(self, seed_num=2022, current_date=None, benchmark_algo=None):

        self.notes = 'MuSA Implementation' 
        if benchmark_algo is None:
            self.benchmark_algo = 'MASA-dc' # Algorithm: 'MASA-dc', 'MASA-mlp', 'MASA-lstm', 'TD3-Profit', 'TD3-PR', 'TD3-SR', 'CRP', 'EG', 'OLMAR', 'PAMR', 'CORN', 'RMR', 'EIIE', 'PPN', 'RAT'
        else:
            self.benchmark_algo = benchmark_algo
        self.market_name = 'SP500' # Financial Index: 'DJIA', 'SP500'       
        self.topK = 10 # Number of assets in a portfolio (10, 20, 30)
        self.num_epochs = 50 # max episode.

        self.is_enable_dynamic_risk_bound = False # True if enabling that the market observer sends info to solver-based agents. 
        self.enable_controller = False # True if enabling the solver-based agent.
        self.enable_market_observer = True # True if enabling the market observer.

        if self.benchmark_algo.split('-')[0] in ['TD3', 'PPO', 'SAC', 'DDPG', 'TRPO']:
            self.rl_model_name = self.benchmark_algo.split('-')[0]
            self.mode = 'RLonly'
            ...
            self.trained_best_model_type = 'max_capital' # 'js_loss', 'max_capital'
            if self.benchmark_algo.split('-')[1] == 'DualAgent':
                self.is_enable_dynamic_risk_bound = False
                self.enable_market_observer = False 
            if 'Corr' in self.mktobs_algo:
                self.mktobs_algo = '{}_1'.format(self.benchmark_algo.split('-')[1][:14]) # 'SentimentMultiCorr' -> 'SentimentMulti_1'
        else:
            # Baseline models
        ...
        if self.market_name == 'SP500':
            self.dc_threshold_lst = [0.1] # 0.01, suggest 0.1
            if 'Corr' in self.benchmark_algo:
                self.senti_k = 1 # optimize the senti_k by corr coeff online learning mechanism.
            else:
                self.senti_k = 0.2 # suggest 0.2, optimized by the training data set.
            self.dc_lookback = 10 # 5, suggest 10, (1, 5, 10, 20, 30)
            self.senti_corr_lookback = 10 # suggest 10, 180, 90, 60, 30, 10, 5
            self.senti_corr_combine = 'sum' # 'sum', 'corrmax'

        elif self.market_name in ['DJIA', 'DOW']:
            self.dc_threshold_lst = [0.05] # suggest 0.1
            if 'Corr' in self.benchmark_algo:
                self.senti_k = 1 # optimize the senti_k by corr coeff online learning mechanism.
            else:
                self.senti_k = 0.2 # suggest 0.2
            self.dc_lookback = 10 # suggest 10, (1, 5, 10, 20, 30)
            self.senti_corr_lookback = 10 # 180, 90, 60, 30, 10, 5
            self.senti_corr_combine = 'sum' # 'sum', 'corrmax'
        else:
            raise ValueError("Unexpected market_name [{}].".format(self.market_name))   

        self.dc_is_change_range = False # True: [0, 1], False [-1, 1]
        self.dc_is_norm_score = False
        ...
        # self.tmp_name = '{}-{}_Cls3_{}_{}_K{}_M{}_{}_{}'.format(self.dc_is_change_range, self.dc_is_norm_score, self.mode, self.mktobs_algo, self.topK, self.period_mode, self.market_name, self.trained_best_model_type)
        self.tmp_name = '{}-{}-{}-DC{}-K{}-LB{}-SLB{}_Cls3_{}_{}_K{}_M{}_{}_{}'.format(self.senti_corr_combine, self.dc_is_change_range, self.dc_is_norm_score, str(self.dc_threshold_lst[0])[-2:], ''.join(str(self.senti_k).split('.')), self.dc_lookback, self.senti_corr_lookback, self.mode, self.mktobs_algo, self.topK, self.period_mode, self.market_name, self.trained_best_model_type)
        ...
        date_split_dict = {            
            1: {'train_date_start': '2010-02-01 00:00:00',
                'train_date_end': '2015-05-31 23:59:59',
                'valid_date_start': '2015-06-01 00:00:00',
                'valid_date_end': '2017-05-31 23:59:59',
                'test_date_start': '2017-06-01 00:00:00 ',
                'test_date_end': '2020-05-31 23:59:59'},
        }
        ...
        self.otherRef_indicator_lst = ['MA-{}'.format(self.otherRef_indicator_ma_window), 'DAILYRETURNS-{}'.format(self.dailyRetun_lookback)] + ['DC-{}'.format(i) for i in range(1, len(self.dc_threshold_lst) + 1)]
    ...
    def load_para(self):
        if self.enable_market_observer:
            if self.rl_model_name == 'TD3':
                if ('Sentiment' in self.benchmark_algo) and (not self.dc_is_change_range):
                    policy_name = 'TD3PolicyAdjUnchange'
                else:
