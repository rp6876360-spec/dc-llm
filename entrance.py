...
def RLcontroller(config):
    ...
    if config.enable_market_observer:
        ...
        elif 'Sentiment' in config.mktobs_algo:
            senti_inst = SentimentGeneration(config=config, price_data=data)
            senti_data = senti_inst.read_sentiment()
            obs_kwargs = {'sentiment_score': senti_data}
            mkt_observer = MarketObserver_Algorithmic(config=config, action_dim=stock_num, **obs_kwargs)
        else:
...
def benchmark_test(config):
    ...
    if (config.valid_date_start is not None) and (config.valid_date_end is not None):
        validInvest_env_para = config.invest_env_para 
        if config.benchmark_algo in config.use_cash_algo_lst:  
            env_valid = StockPortfolioEnv_cash(
                config=config, rawdata=data_dict['valid'], mode='valid', stock_num=stock_num, action_dim=stock_num+1, # +1 for cash action 
                tech_indicator_lst=tech_indicator_lst, **validInvest_env_para
            )
        else: 
            env_valid = StockPortfolioEnv(
    ...
    if (config.test_date_start is not None) and (config.test_date_end is not None):
        ...
        if config.benchmark_algo in config.use_cash_algo_lst:  
            env_test = StockPortfolioEnv_cash(
                config=config, rawdata=data_dict['test'], mode='test', stock_num=stock_num, action_dim=stock_num+1, # +1 for cash action 
                tech_indicator_lst=tech_indicator_lst, **testInvest_env_para
            )
        else:
            env_test = StockPortfolioEnv(
    ...
    model_kwargs = {'raw_data': data, 'extra_train': data_dict['extra_train'], 'extra_valid': data_dict['extra_valid'], 'extra_test': data_dict['extra_test']}
    ModelCls = benchmark_algo_select(model_name=config.benchmark_algo)
    po_model = ModelCls(env=env_test, env_valid=env_valid, **model_kwargs)
    po_model.run()
    del po_model

def entrance():
    """
    Entrance function for running the MuSA framework
    """
    runtimes = 1
    # 'TD3-Profit', 'TD3-PR', 'TD3-SR',
    # 'DC', 'DC-SentimentMulti', 'DC-SentimentMultiCorr', 'DC-SentimentBenzinga', 'DC-SentimentNonBenzinga'
    # Algorithm: 'MASA-SentimentMulti', 'MASA-SentimentMultiCorr', 'MASA-SentimentBenzinga', 'MASA-SentimentNonBenzinga', 'MASA-DualAgent', 'MASA-dc', 'MASA-mlp', 'MASA-lstm', 
    """
    Single-agent + sentiment (eq to dual agent, False DCM, False ctrl, True mkt_obs, MASA-XX)
    Dual-agent + sentiment (eq to Triple agent, True DCM, True-ctrl, True mkt_obs, MASA-XX)
    """
    """
    TD3+Ctrl+SentimentMulti
    TD3+Ctrl+SentimentNonBenzinga/SentimentBenzinga
    TD3+SentimentMulti
    TD3+SentimentNonBenzinga/SentimentBenzinga
    DJIA
    """

    # for benchmark_algo in ['DC', 'DC-SentimentBenzinga', 'DC-SentimentNonBenzinga', 'DC-SentimentMulti', 'DC-SentimentMultiCorr']: 
    # for benchmark_algo in ['DC', 'DC-SentimentBenzinga', 'DC-SentimentNonBenzinga', 'DC-SentimentMulti']: 
    for benchmark_algo in ['MASA-DualAgent']: 
        for run_no in range(1, runtimes+1):
            ...
            config = Config(seed_num=rand_seed, current_date=current_date, benchmark_algo=benchmark_algo) 
            config.print_config()
            ...
            print("[Done] Run No.: {}, Total cputime: {} s, system time: {} s".format(run_no, np.round(end_cputime - start_cputime, 2), np.round(end_systime - start_systime, 2)))
            post_process(config=config)
            print("[End] benchmark_algo: {}, run_no: {}".format(benchmark_algo, run_no))
