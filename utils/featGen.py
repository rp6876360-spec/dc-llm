    ...
    def gen_feat(self, data):
        ...
        for indidx, sigIndicatorName in enumerate(list(self.config.tech_indicator_input_lst) + list(self.config.otherRef_indicator_lst)):
            ...
            for sigStockName in stock_lst:
                ...
                elif 'DC' in sigIndicatorName:
                    temp = {}
                    close_ay = np.array(dataSig['close'])
                    dc_event_lst = dc_feature_generation(data=close_ay, dc_threshold=self.config.dc_threshold_lst[int(sigIndicatorName.split('-')[-1])-1])
                    dc_event_lst = np.array(dc_event_lst).astype(np.intc)
                    temp[sigIndicatorName] = dc_event_lst # 1 for uptrend, 0 for downtrend
