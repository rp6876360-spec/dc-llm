...
class StockPortfolioEnv(gym.Env):
    ...
    def __init__(self, config, rawdata, mode, stock_num, action_dim, tech_indicator_lst, max_shares,
        ...
        self.cur_date = self.curData['date'].unique()[0]
        self.date_memory = [self.cur_date]
        self.reward_lst = [0]
    ...
    def step(self, actions):
        ...
        if self.terminal:
            self.rl_reward_profit_lst = np.array(self.rl_reward_profit_lst)
            rl_reward_profit_lst_length = np.array(self.rl_reward_profit_lst[self.rl_reward_profit_lst!=0])
            if len(rl_reward_profit_lst_length) != 0:
                print("profit mean: {}, max: {}, min: {}, median: {}, sum: {}".format(np.mean(self.rl_reward_profit_lst), np.max(self.rl_reward_profit_lst), np.min(self.rl_reward_profit_lst), np.median(self.rl_reward_profit_lst), np.sum(self.rl_reward_profit_lst)))

            self.rl_reward_risk_lst = np.array(self.rl_reward_risk_lst)
            rl_reward_risk_lst_length = np.array(self.rl_reward_risk_lst[self.rl_reward_risk_lst!=0])
            if len(rl_reward_risk_lst_length) != 0:
                print("risk mean: {}, max: {}, min: {}, median: {}, sum: {}".format(np.mean(self.rl_reward_risk_lst), np.max(self.rl_reward_risk_lst), np.min(self.rl_reward_risk_lst), np.median(self.rl_reward_risk_lst), np.sum(self.rl_reward_risk_lst)))
            print("reward mean: {}, max: {}, min: {}, median: {}, sum: {}".format(np.mean(self.reward_lst), np.max(self.reward_lst), np.min(self.reward_lst), np.median(self.reward_lst), np.sum(self.reward_lst)))
            ...
            self.cur_date = self.curData['date'].unique()[0]
            self.date_memory.append(self.cur_date)
    ...
    def save_profile(self, invest_profile):
        ...
        if True:
            ...
            log_str = "Mode: {}, Ep: {}, Current epoch capital: {}, historical best captial ({} ep): {} | AR: {} | cputime cur: {} s, avg: {} s, system time cur: {} s/ep, avg: {} s/ep..".format(self.mode, self.epoch, self.cur_capital, v_ep, v, np.round(np.array(phist_df['annualReturn_pct'])[-1], 4), np.round(np.array(phist_df['cputime'])[-1], 2), np.round(cputime_avg, 2), np.round(np.array(phist_df['systime'])[-1], 2), np.round(systime_avg, 2))
            print(log_str)
    ...
    def run_mkt_observer(self, stage=None, rate_of_price_change=None):
        ...
        if self.config.enable_market_observer:
            ...
            input_kwargs = {'mode': self.mode, 'stock_ma_price': np.array([stock_ma_price]), 'stock_cur_close_price': np.array([stock_cur_close_price]), 
                            'dc_events': np.array([dc_events]), 'cur_date': cur_date}
